...
def run_length(L: int, *, full_pdb: str,
    ...
    with _quiet():
        ...
        engine.attach_reporters(cfg, ctx.simulation, append=append,
                                total_steps=cfg.md_steps)
        # On a restart, setup_simulation set nsteps_remain = md_steps - checkpoint steps
        # (0 if the cumulative target was already reached); a fresh run gets md_steps.
        if ctx.nsteps_remain > 0:
    ...
    if restart and ctx.nsteps_remain <= 0:
        print(f"  L={L:>3d}  {(label or 'run'):<28s}  cumulative {cfg.md_steps} steps already "
...
def run_cylinder_synthesis(full_pdb: str, *, L0: int = 1, L_max: Optional[int] = None,
    ...
    # it. Continues the length-L_max system from the previous final; its own progress unit
    # (skipped on resume if already done); its final structure seeds the ejection phase.
    if params.stall_steps > 0:
        if do_resume and prog.is_done("stall"):
            ...
            print(f"=== Stall (L = {L_max}, {params.stall_steps} steps, "
                  f"C-terminus restraint ON -> held at PTC) -> {out_path / 'stall'}/ ===")
            ...
            prev_final = run_length(
                ...
                restrain=True, out_subdir="stall",
...
def read_cylinder_config(config_file: str, verbose: bool = True) -> CylinderConfig:
    ...
    - **Post-synthesis** (run in order stall -> ejection): ``stall_steps`` -- hold at the
      PTC with the C-terminus restraint still ON (ribosome stalling; 0 = skip);
      ``ejection_steps`` -- free run with the C-terminus restraint released (0 = skip).
...
def tunnel_tcl(config_file: str, wall_outer_nm: Optional[float] = None) -> str:
    ...
    can be appended to a movie ``.tcl`` (see ``cosmo-csp-movie --tunnel``).
...
# Auto-generated by cosmo.csp.cylinder.tunnel_tcl from the run's cylinder.ini.
